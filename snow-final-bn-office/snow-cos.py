...
def evaluate_accuracy(test_data, net, device, args):
    ...
    with torch.no_grad():
        for i in range(len(test_data)):
            ...
            weight = torch.zeros(10, 512).to(device)
            snow_bw = args.weight_regular(net.snow)
            for k in range(10):
                weight[k] = snow_bw[k]
            y_hat = std_fc(features, weight, std_regular=True)
            acc_sum += ((y_hat.argmax(dim=1) % 10) == (y.to(device).argmax(dim=1) % 10)).float().sum().cpu().item()
            net.train()
            n += y.shape[0]
    return acc_sum / n
...
def train(net, batch_size, optimizer, device, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(len(train_data)):
            ...
            snow_bw = args.weight_regular(net.snow)
            for k in range(30):
                weight[k] = snow_bw[k % 10] + args.alpha * snow_bw[k + 7]
            y_hat = std_fc(features, weight, std_regular=True)
...
if __name__ == '__main__':
    ...
    train_root = '../Dataset/office_caltech_10_C30/train'
    test_root = '../Dataset/office_caltech_10_C30/test'
    ...
    alpha_list = []
    id_acc = torch.zeros(6, 3)
    ood_acc = torch.zeros(6, 3)
    for alpha_value in [0.35]:
        args.alpha = alpha_value / 5
        alpha_list.append(args.alpha)
        for seed in range(5):
            for num_groups, num_iterations in [[0, 0]]:
                ...
                id_acc[seed][alpha_value] = result[2]
                ood_acc[seed][alpha_value] = result[3]
